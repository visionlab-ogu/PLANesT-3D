...
def shuffle_data(data, labels):
    """ Shuffle data and labels.
        Input:
          ...
          label: B,... numpy array
          eigen: B,N, 18 numpy array
        Return:
          shuffled data, label, eigen and shuffle indices
    """
...
class Dataset(object):
    ...
    def get_data(self, index=0):
        data = self.current_data[index:(index+1), 0:self.npoints, :].copy()
        label = self.current_label[index:(index+1)].copy()
        return data, label
        
        
class DatasetOnePC(object):
    def __init__(self, list_filename, npoints = 1024, shuffle=False):
        self.list_filename = list_filename
        self.npoints = npoints
        self.h5_files = getDataFiles(self.list_filename)
        self.shuffle = shuffle
    ...
    def get_data(self, idx):
        ...
        for filename in self.h5_files:
            data, label = load_h5(filename)
            current_data.append(data)
            current_label.append(label)
        self.current_data,self.current_label = np.concatenate(current_data,0), np.concatenate(current_label,0)
        self.current_label = np.squeeze(self.current_label)
        if self.shuffle:
            self.current_data = provider.shuffle_points(self.current_data)
        data_batch = self.current_data[idx, 0:self.npoints, :].copy()
        label_batch = self.current_label[idx].copy()
        return data_batch, label_batch
        

if __name__=='__main__':
    d = Dataset('/media/visionlab/Data/KT/pointnet2/data/IntrA_hdf5_2048/test_files_eigen.txt')
    print(d.shuffle)
    print(d.has_next_batch())
    ps_batch, cls_batch = d.next_batch(False)
