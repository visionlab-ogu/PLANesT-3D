...
def combined_module(npoint, radius, nsample, xyz, points, use_xyz, bn_decay, is_training, scope, center_flag, radius_flag):     
    ...
    if center_flag==True and radius_flag==False:
        ...
        radius_adap = radius_update_module(npoint, radius, nsample, xyz, new_xyz, points, new_points, bn_decay, is_training, scope, agg_type='weighted_cum', module='attention')
        shifted_xyz = new_xyz
        ...
        shifted_xyz, shift_val, sa_feat = center_shift_module(npoint, radius, nsample, xyz, new_xyz, points, new_points, bn_decay, is_training, scope, method='CSI-I') 
        #_,idx_knn = knn_point(1, xyz, shifted_xyz)
        #shifted_xyz = tf.squeeze(group_point(xyz, idx_knn),2)
        radius_adap = radius_update_module(npoint, radius, nsample, xyz, shifted_xyz, points, tf.squeeze(sa_feat,2), bn_decay, is_training, scope, agg_type='weighted_cum', module='attention')
...
def center_shift_module(npoint, radius, nsample, xyz, center_xyz, points, center_points, bn_decay, is_training, scope, method):       
    ...
    if method == 'CSA-I':
        ...
        shift, sa_feat = method_CSA_II(center_points, grouped_points, grouped_xyz_rel, radius, bn_decay, is_training, relation = 'sub')
    elif method == 'CSA-III':
