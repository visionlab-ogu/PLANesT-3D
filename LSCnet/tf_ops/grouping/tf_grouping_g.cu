...
__global__ void query_ball_point_random_gpu(int b, int n, int m, float radius, int nsample, const float *xyz1, const float *xyz2, const int *rand_id, int *idx, int *pts_cnt) {
    int batch_index = blockIdx.x;
    xyz1 += n*3*batch_index;
    xyz2 += m*3*batch_index;
    rand_id += m*n*batch_index;
    idx += m*nsample*batch_index;
    pts_cnt += m*batch_index; // counting how many unique points selected in local region
    ...
    for (int j=index;j<m;j+=stride) {
        ...
        for (int k=0;k<n;++k) {
            ...
            float y2=xyz2[j*3+1];
            float z2=xyz2[j*3+2];
            float x1=xyz1[id*3+0];
            float y1=xyz1[id*3+1];
            float z1=xyz1[id*3+2];
    	    float d=max(sqrtf((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)+(z2-z1)*(z2-z1)),1e-20f);
            if (d<radius) {
                if (cnt==0) { // set ALL indices to k, s.t. if there are less points in ball than nsample, we still have valid (repeating) indices
                    for (int l=0;l<nsample;++l)
...
__global__ void query_ball_point_pointwise_gpu(int b, int n, int m, const float *radius, int nsample, const float *xyz1, const float *xyz2, int *idx, int *pts_cnt) {
    int batch_index = blockIdx.x;
    xyz1 += n*3*batch_index;
    xyz2 += m*3*batch_index;
    radius += m*1*batch_index;
    idx += m*nsample*batch_index;
    pts_cnt += m*batch_index; // counting how many unique points selected in local region
...
void queryBallPointRandomLauncher(int b, int n, int m, float radius, int nsample, const float *xyz1, const float *xyz2, const int *rand_id, int *idx, int *pts_cnt) {
    query_ball_point_random_gpu<<<b,256>>>(b,n,m,radius,nsample,xyz1,xyz2,rand_id,idx,pts_cnt);
    //cudaDeviceSynchronize();
}
void queryBallPointPointwiseLauncher(int b, int n, int m, const float *radius, int nsample, const float *xyz1, const float *xyz2, int *idx, int *pts_cnt) {
    query_ball_point_pointwise_gpu<<<b,256>>>(b,n,m,radius,nsample,xyz1,xyz2,idx,pts_cnt);
    //cudaDeviceSynchronize();
}
