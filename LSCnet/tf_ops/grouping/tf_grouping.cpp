...
REGISTER_OP("QueryBallPointRandom")
    .Attr("radius: float")
    .Attr("nsample: int")
    .Input("xyz1: float32")
    .Input("xyz2: float32")
...
REGISTER_OP("QueryBallPointPointwise")
    .Attr("nsample: int")
    .Input("xyz1: float32")
    .Input("xyz2: float32")
...
void queryBallPointRandomLauncher(int b, int n, int m, float radius, int nsample, const float *xyz1, const float *xyz2, const int *rand_ind, int *idx, int *pts_cnt);
class QueryBallPointRandomGpuOp : public OpKernel {
    public:
        explicit QueryBallPointRandomGpuOp(OpKernelConstruction* context) : OpKernel(context) {
            OP_REQUIRES_OK(context, context->GetAttr("radius", &radius_));
            OP_REQUIRES(context, radius_ > 0, errors::InvalidArgument("QueryBallPoint expects positive radius"));
        ...
        void Compute(OpKernelContext* context) override {
            ...
            const Tensor& rand_tensor = context->input(2);
            OP_REQUIRES(context, rand_tensor.dims()==3 && rand_tensor.shape().dim_size(2)==n, errors::InvalidArgument("QueryBallPoint expects (batch_size, npoint, ndataset) rand_id shape."));
            ...
            auto xyz2_flat = xyz2_tensor.flat<float>();
            const float *xyz2 = &(xyz2_flat(0));
            auto rand_id_flat = rand_tensor.flat<int>();
            const int *rand_id = &(rand_id_flat(0));
            auto idx_flat = idx_tensor->flat<int>();
            int *idx = &(idx_flat(0));
            auto pts_cnt_flat = pts_cnt_tensor->flat<int>();
            int *pts_cnt = &(pts_cnt_flat(0));
            queryBallPointRandomLauncher(b,n,m,radius_,nsample_,xyz1,xyz2,rand_id, idx, pts_cnt);
        }
    private:
        float radius_;
        int nsample_;
};
REGISTER_KERNEL_BUILDER(Name("QueryBallPointRandom").Device(DEVICE_GPU), QueryBallPointRandomGpuOp);

void queryBallPointPointwiseLauncher(int b, int n, int m, const float *radius, int nsample, const float *xyz1, const float *xyz2, int *idx, int *pts_cnt);
class QueryBallPointPointwiseGpuOp : public OpKernel {
    public:
        explicit QueryBallPointPointwiseGpuOp(OpKernelConstruction* context) : OpKernel(context) {
            /*OP_REQUIRES_OK(context, context->GetAttr("radius", &radius_));
            OP_REQUIRES(context, radius_ > 0, errors::InvalidArgument("QueryBallPoint expects positive radius"));*/
        ...
        void Compute(OpKernelContext* context) override {           
            ...
            OP_REQUIRES(context, radius_tensor.dims()==3 && radius_tensor.shape().dim_size(2)==1, errors::InvalidArgument("QueryBallPoint expects (batch_size, ndataset) radius shape."));
            ...
            auto pts_cnt_flat = pts_cnt_tensor->flat<int>();
            int *pts_cnt = &(pts_cnt_flat(0));
            queryBallPointPointwiseLauncher(b,n,m,radius,nsample_,xyz1,xyz2,idx,pts_cnt);
        }
    private:
        //float radius_;
        int nsample_;
};
REGISTER_KERNEL_BUILDER(Name("QueryBallPointPointwise").Device(DEVICE_GPU), QueryBallPointPointwiseGpuOp);
