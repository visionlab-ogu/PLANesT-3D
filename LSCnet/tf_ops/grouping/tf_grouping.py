...
def query_ball_point_random(radius, nsample, xyz1, xyz2, rand_id):
    '''
    Input:
        radius: float32, ball search radius
        nsample: int32, number of points selected in each ball region
    ...
    return grouping_module.query_ball_point_random(xyz1, xyz2, rand_id, radius, nsample)
ops.NoGradient('QueryBallPointRandom')
def query_ball_point_pointwise(radius, nsample, xyz1, xyz2):
    '''
    Input:
        radius: (batch_size, npoint,1) float32, ball search radius
        nsample: int32, number of points selected in each ball region
        xyz1: (batch_size, ndataset, 3) float32 array, input points
        xyz2: (batch_size, npoint, 3) float32 array, query points
    Output:
    ...
    return grouping_module.query_ball_point_pointwise(xyz1, xyz2, radius, nsample)
ops.NoGradient('QueryBallPointPointwise')
...
if __name__=='__main__':
    ...
    with tf.Session('') as sess:
        now = time.time() 
        for _ in range(100):
            ret,pts = sess.run([grouped_points, xyz1])
        print time.time() - now
